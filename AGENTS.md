...
- 新需求必须从最新基线新建功能分支开始实现，不要直接在 `main` 或 `dev` 上开发；需求实现并完成验证后，推送该功能分支到远端，再按项目流程合并到 `main` 分支。
- 如果用户明确要求“推送到 dev”或“合并到 dev”，不要只停留在功能分支；完成验证后需要把当前任务改动合并回 `dev` 并推送到 `origin/dev`。
