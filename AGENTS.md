...
- 纯文档/方案/说明类任务必须走轻量流程：如果只修改 `docs/`、`README`、规则/规范文字、设计方案、排查记录或计划文档，且不修改代码、运行时配置、依赖、构建脚本、测试、发布/部署文件，也不会改变项目运行行为，则只做本地落盘和轻量校验（例如 `git diff --check`、敏感信息自查、必要的文本格式检查）。这类变更需要进入 `dev` 时，可以按用户要求直接提交/推送到 `dev` 或将文档分支快速合并到 `dev`；不要默认开 PR、等待 GitHub Actions 或执行重负载验证。只有实际修改代码或影响运行/构建/测试/部署行为时，才进入完整分支、PR、CI、合并流程。
...
- 涉及前端 UI、布局、响应式、交互密度调整，或任何需要看真实页面效果的 Web 测试/验收任务，完成代码验证后必须优先依赖 `chrome:Chrome` 打开真实 Chrome 页面检查，至少覆盖桌面和移动版，确认没有溢出、重叠、错位或被裁切后再结束任务。不要默认使用 `computer-use` 或 Playwright 做这类验收；只有用户明确要求，或 Chrome 插件不可用/不可达时，才把 `computer-use` 或 Playwright 作为后备，并在交付说明里写明原因。
