...
## 2026-05-29 · 文档与工作流规范集中更新

### 背景

项目已有的 `AGENTS.md`（AI 入口索引）和 `CLAUDE.md`（项目级规范）存在内容不够同步、CLAUDE.md 未被 git 跟踪等问题。需要做一次文档维护，确保规范文件可追溯、索引覆盖完整。

### 结论

- 将 `CLAUDE.md` 从项目根目录纳入 codeProxy git 仓库，确保项目级规范可版本管理
- 更新 `AGENTS.md`，增加跨仓库项目概览上下文，引用 CLAUDE.md 和 CliRelay AGENTS.md
- 新增 `docs/` 分支命名约定，用于纯文档/规范变更的隔离
- 后续文档维护可直接在 `docs/*` 分支上操作，避免与代码功能分支混淆

### 影响范围

- 新增 `CLAUDE.md` 进入 codeProxy 仓库根目录，未来修改需同步两个仓库的配置信息
- `AGENTS.md` 补充项目跨仓库上下文的引用，Agent 首次接入时应能理解 CliProxy 全貌
- 规则文件与演进记录同步更新

### 回滚策略（如适用）

回滚本次 commit 即可恢复全部文档文件到之前状态。
